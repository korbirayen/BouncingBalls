...
from util import utils, CATEGORY_CAR_BOX, CATEGORY_BALL, CATEGORY_CAR


class Car:
    def __init__(self,team,pos,radius = 64/10,color = (255,255,255),vel = Vector2(0,-0)):
        self.team = team
        self.color = color
        self.radius = radius
        ...
        self.circle_shape = self.circle_body.CreateCircleFixture(radius=self.radius, density=1, friction=0.0, restitution=1.00,
                                                                 filter=b2Filter(categoryBits=CATEGORY_CAR, maskBits=(CATEGORY_BALL | CATEGORY_CAR | CATEGORY_CAR_BOX)))
        self.circle_body.linearVelocity = vel
        self.circle_body.userData = self
        if self.team == 'red':
            self.oImg = pygame.image.load("assets/u1.png")
        else:
            self.oImg = pygame.image.load("assets/c.png")
