...
class Ring:
    ...
    def shrink_radius(self):
        for box in self.boxes:
            utils.world.DestroyBody(box.box_body)
        self.boxes = []
        self.radius -= utils.deltaTime() * 40
        # for i in range(0, self.cRange):
        #     rad = math.radians(i)
        #     pos = self.center + Vector2(math.cos(rad) * self.radius, math.sin(rad) * self.radius)
        #     self.boxes.append(Box(Vector2(pos.x, pos.y), self.color))
        #     self.initial_positions.append(Vector2(pos.x, pos.y) - self.center)
    ...
    def draw(self):
        for box in self.boxes:
            if box.destroyFlag:
                continue
            box.draw()
        pygame.draw.circle(utils.screen, self.color, Vector2(utils.width/2,utils.height/2), self.radius,4)
