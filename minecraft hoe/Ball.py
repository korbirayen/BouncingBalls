...
class Ball:
    def __init__(self,pos,radius = 1,color = (255,255,255),vel = Vector2(-5.2,-3)):
        self.color = color
        self.radius = radius
        ...
        self.currentImge = 1
        self.oImg = pygame.image.load("assets/"+ str(self.currentImge) +".png")
        self.img = pygame.transform.scale(self.oImg,(self.radius*2*10,self.radius*2*10))
        self.circle_body.angularVelocity = 5
    ...
    def draw(self):
        ...
        if self.currentImge + 1 < 7:
            img = pygame.image.load("assets/" + str(self.currentImge+1) + ".png")
            nextImg = pygame.transform.scale(img, (self.radius * 1.5 * 10, self.radius * 1.5 * 10))
            utils.screen.blit(nextImg,(470,35))
