...
class MyContactListener(b2ContactListener):
    ...
    def BeginContact(self, contact):
        ...
        if (isinstance(bodyA.userData,Box) and isinstance(bodyB.userData,Ball))\
                or (isinstance(bodyA.userData,Ball) and isinstance(bodyB.userData,Box)):
            self.collisions.append((bodyA, bodyB))
        if (isinstance(bodyA.userData,Ball) and isinstance(bodyB.userData,Ball))\
                or (isinstance(bodyA.userData,Ball) and isinstance(bodyB.userData,Ball)):
            self.collisions.append((bodyA, bodyB))
